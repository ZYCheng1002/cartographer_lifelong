...
void RunOfflineNode(const MapBuilderFactory& map_builder_factory, rclcpp::Node::SharedPtr cartographer_offline_node) {
  ...
  std::vector<std::string> configuration_basenames(
      std::sregex_token_iterator(FLAGS_configuration_basenames.begin(), FLAGS_configuration_basenames.end(), regex, -1),
      std::sregex_token_iterator());
  ...
  Node node(node_options, std::move(map_builder), tf_buffer, cartographer_offline_node, FLAGS_collect_metrics);
  ...
  for (size_t current_bag_index = 0; current_bag_index < bag_filenames.size(); ++current_bag_index) {
    ...
    for (const auto& expected_sensor_id : bag_expected_sensor_ids.at(current_bag_index)) {
      ...
      const auto bag_resolved_topic = std::make_pair(static_cast<int>(current_bag_index), "/" + expected_sensor_id.id);
      if (bag_topic_to_sensor_id.count(bag_resolved_topic) != 0) {
        LOG(ERROR) << "Sensor /" << expected_sensor_id.id << " of bag " << current_bag_index << " resolves to topic "
              ...
              try {
                ...
                for (auto& transform : tf_message.transforms) {
                  try {
                    ...
                    // always work.
                    tf_buffer->setTransform(transform, "unused_authority", msg->topic_name == kTfStaticTopic);
  ...
  while (playable_bag_multiplexer.IsMessageAvailable()) {
...
#else
    if (msg.recv_timestamp < (begin_time.nanoseconds() + rclcpp::Duration(FLAGS_skip_seconds, 0).nanoseconds())) {
      continue;
    }
#endif
    ...
    if (bag_index_to_trajectory_id.count(bag_index) == 0) {
      ...
      CHECK(
          bag_index_to_trajectory_id
              .emplace(std::piecewise_construct, std::forward_as_tuple(bag_index), std::forward_as_tuple(trajectory_id))
              .second);
    ...
    if (it != bag_topic_to_sensor_id.end()) {
      ...
      if (topic_type == "sensor_msgs/msg/LaserScan") {
        ...
        rclcpp::SerializedMessage serialized_msg(*msg.serialized_data);
        sensor_msgs::msg::PointCloud2::SharedPtr pcl2_scan_msg = std::make_shared<sensor_msgs::msg::PointCloud2>();
        ...
        rclcpp::SerializedMessage serialized_msg(*msg.serialized_data);
        sensor_msgs::msg::Imu::SharedPtr imu_scan_msg = std::make_shared<sensor_msgs::msg::Imu>();
  ...
  if (rclcpp::ok() && !(bag_filenames.empty() && FLAGS_save_state_filename.empty())) {
    const std::string state_output_filename =
        FLAGS_save_state_filename.empty() ? bag_filenames.front() + ".pbstream" : FLAGS_save_state_filename;
