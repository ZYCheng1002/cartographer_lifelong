...
class OptimizationProblem2D : public OptimizationProblemInterface<NodeSpec2D, SubmapSpec2D, transform::Rigid2d> {
 public:
  ...
  const MapById<SubmapId, SubmapSpec2D>& submap_data() const override { return submap_data_; }
  const std::map<std::string, transform::Rigid3d>& landmark_data() const override { return landmark_data_; }
  const sensor::MapByTime<sensor::ImuData>& imu_data() const override { return empty_imu_data_; }
  const sensor::MapByTime<sensor::OdometryData>& odometry_data() const override { return odometry_data_; }
  ///@brief 添加固定帧
  ...
  const sensor::MapByTime<sensor::FixedFramePoseData>& fixed_frame_pose_data() const { return fixed_frame_pose_data_; }
 ...
 private:
  ...
  MapById<NodeId, NodeSpec2D> node_data_;                                // todo 如何理解
  MapById<SubmapId, SubmapSpec2D> submap_data_;                          // 子地图
  std::map<std::string, transform::Rigid3d> landmark_data_;              // 路标点
  sensor::MapByTime<sensor::ImuData> empty_imu_data_;                    // imu, not used
  sensor::MapByTime<sensor::OdometryData> odometry_data_;                // dr
  sensor::MapByTime<sensor::FixedFramePoseData> fixed_frame_pose_data_;  // 固定位姿
