...
class PoseGraph2D : public PoseGraph {
 public:
  ...
  NodeId AddNode(std::shared_ptr<const TrajectoryNode::Data> constant_data,
                 int trajectory_id,
                 const std::vector<std::shared_ptr<const Submap2D>>& insertion_submaps) LOCKS_EXCLUDED(mutex_);
  ...
  void SetLandmarkPose(const std::string& landmark_id,
                       ...
                       const bool frozen = false) override LOCKS_EXCLUDED(mutex_);
  ...
  void SetInitialTrajectoryPose(int from_trajectory_id,
                                int to_trajectory_id,
                                const transform::Rigid3d& pose,
                                const common::Time time) override LOCKS_EXCLUDED(mutex_);
 ...
 private:
  ...
  void AddWorkItem(const std::function<WorkItem::Result()>& work_item) LOCKS_EXCLUDED(mutex_)
  ...
  // structures.
  NodeId AppendNode(std::shared_ptr<const TrajectoryNode::Data> constant_data,
                    int trajectory_id,
  ...
  std::vector<SubmapId> InitializeGlobalSubmapPoses(
      int trajectory_id, const common::Time time, const std::vector<std::shared_ptr<const Submap2D>>& insertion_submaps)
      EXCLUSIVE_LOCKS_REQUIRED(mutex_);
  ...
  void HandleWorkQueue(const constraints::ConstraintBuilder2D::Result& result) LOCKS_EXCLUDED(mutex_)
      LOCKS_EXCLUDED(work_queue_mutex_);
  ...
  /// Runs the optimization. Callers have to make sure, that there is only one optimization being run at a time.
  void RunOptimization() LOCKS_EXCLUDED(mutex_);

  bool CanAddWorkItemModifying(int trajectory_id) EXCLUSIVE_LOCKS_REQUIRED(mutex_);

  /// Computes the local to global map frame transform based on the given 'global_submap_poses'.
  ...
  /// If it exists, further work items must be added to this queue, and will be considered later.
  ...
  common::ThreadPool* const thread_pool_;  // 线程池
