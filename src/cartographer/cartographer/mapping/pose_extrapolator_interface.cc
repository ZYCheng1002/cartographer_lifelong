...
proto::ImuBasedPoseExtrapolatorOptions CreateImuBasedPoseExtrapolatorOptions(
  ...
  *options.mutable_solver_options() =
      CreateCeresSolverOptionsProto(parameter_dictionary->GetDictionary("solver_options").get());
...
proto::PoseExtrapolatorOptions CreatePoseExtrapolatorOptions(
  ...
  *options.mutable_constant_velocity() =
      CreateConstantVelocityPoseExtrapolatorOptions(parameter_dictionary->GetDictionary("constant_velocity").get());
  *options.mutable_imu_based() =
      CreateImuBasedPoseExtrapolatorOptions(parameter_dictionary->GetDictionary("imu_based").get());
...
std::unique_ptr<PoseExtrapolatorInterface> PoseExtrapolatorInterface::CreateWithImuData(
  ...
  if (options.use_imu_based()) {
    return ImuBasedPoseExtrapolator::InitializeWithImu(options.imu_based(), imu_data, initial_poses);
  } else {
    return PoseExtrapolator::InitializeWithImu(common::FromSeconds(options.constant_velocity().pose_queue_duration()),
